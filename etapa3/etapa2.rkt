...
;  (if (member x (take pref-list (index-of pref-list y)))
;      #t
;      #f
;      )
;  )
(define (preferable? pref-list x y)
  (if (member y (drop pref-list (index-of pref-list x)))
      #t
      #f
      )
  )
...
;(define (help pair name)
;  (if(eq? (car pair) name)
;     #t
;     #f
;  ))
(define (get-partner engagements person)
  (define this-find (find-first (lambda (x)
                                  (if(eq? (car x) person)
                                     #t
                                     #f
                                     )
                                  )engagements))
  (if (not(false? this-find))
      (cdr this-find)
      #f
      ))
...
(define (change-helper p L acc val)
  (cond   ((null? L) acc)
          ((and (p (car L)) (not (member val  acc)))   (change-helper p (cdr L) (append acc (list val)) val))   
          (else (change-helper p (cdr L) (append acc (list (car L))) val))
        
          ))

(define (change-first p L val)
  (change-helper p L '() val)
  )
...
(define (update-engagements engagements p1 p2)
  ...
  (change-first pred engagements (cons p1 p2))
  )
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if (eq? (car p1-list) p2)
      #f
      (if (and
           (preferable? p1-list (car p1-list) p2)
...
(define (stable-match? engagements mpref wpref)
  (define (functie logodna)
    (if(better-match-exists? (cdr logodna) (car logodna) (get-pref-list mpref (cdr logodna)) wpref engagements) 
       #f
       #t)
    )
  
  (if(member #f (map functie engagements)) #f
     #t)
